#include "../include/working_on_pkg/Grid.h"
#include <GL/gl.h>
#include <algorithm>
...
Grid::Grid(std::string obj_class){
    ...
    for(unsigned int j = 0; j < num_cells_in_row_; ++j){
        for(unsigned int i = 0; i < num_cells_in_row_; ++i){
            ...
            //my_map_[index].working_on_pkg_value.clear();
            my_map_[index].working_on_pkg_value = 0;
...
void Grid::drawCell(unsigned int n){
    ...
    case 0: //simple map
        ...
        break;
    case 1:
        if(my_map_[n].working_on_pkg_value != 0){                
            glColor3f(1, 1 - my_map_[n].working_on_pkg_value, 0); //R,G,B
    ...
    case 2:
        if(my_map_[n].working_on_pkg_value != 0){                
            glColor3f(1, my_map_[n].working_on_pkg_value, 0); //R,G,B
        }else if(my_map_[n].value == 100)
            glColor3f(0, 0, 0);
        else if(my_map_[n].value == 0)
            glColor3f(0, .95, 0);
        ...
        break;        
    case 3:
/*        if(!my_map_[n].working_on_pkg_value.empty()){
             float sum = 0;
            for(auto i : my_map_[n].working_on_pkg_value)
                sum += i; */
        if(my_map_[n].working_on_pkg_value != 0 && my_map_[n].object_name == goal_object_class){                
            glColor3f(1, 1 - my_map_[n].working_on_pkg_value, 0); //R,G,B
        //if(my_map_[n].working_on_pkg_value != 0){
            //glColor3f(1, my_map_[n].working_on_pkg_value, 0);                
        }else if(my_map_[n].value == 100)
            glColor3f(0, 0, 0);
        else if(my_map_[n].value == 0)
            glColor3f(0, .95, 0);
