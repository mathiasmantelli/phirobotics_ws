#include "../include/working_on_pkg/SemanticHP.h"
    ...
    if(!*updating_grid_now_){
        ...
        for(int j = map_size.min_y; j <= map_size.max_y; j++){
            for(int i = map_size.min_x; i <= map_size.max_x; i++){ 
                ...
                if(current_cell->value == 0 and current_cell->object_name == goal_object_class_ and current_cell->working_on_pkg_value != 0){
        ...
        for(int n = 0; n < offset_size_; n++){
            ...
            if(new_x > 0 and new_x < width and new_y > 0 and new_y < height){
                ...
                neighboor_cell->object_name == goal_object_class_){
                    ...
                    //for(int i = 0; i < neighboor_cell->working_on_pkg_value.size(); i++)
                        //sum += neighboor_cell->working_on_pkg_value[i];
                        sum += 1 - neighboor_cell->working_on_pkg_value;
                }
            }
        }
    }
        ...
        for(int n = 0; n < offset_size_; n++){
            ...
            if(new_x > 0 and new_x < width and new_y > 0 and new_y < height){
                ...
                neighboor_cell->object_name == goal_object_class_){
                    ...
                    sum += neighboor_cell->working_on_pkg_value;
                }
            }
        }
    }
